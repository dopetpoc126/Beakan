package com.example.livemedia.media

import com.example.livemedia.R
...
enum class MusicProvider(
    ...
    UNKNOWN(
        "Unknown Player",
        "unknown.package",
        R.drawable.apple_music // Fallback? Or generic? Using apple_music as per reference for now or better ic_media_play? Reference used apple_music as UNKNOWN fallback.
    ),
    SPOTIFY(
        "Spotify",
        "com.spotify.music",
    ...
    companion object {
        ...
        fun getByPackageName(packageName: String): MusicProvider {
            return entries.firstOrNull { it.packageName == packageName } ?: UNKNOWN
        }
    }
}
